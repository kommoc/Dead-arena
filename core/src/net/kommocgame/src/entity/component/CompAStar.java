...
public class CompAStar implements Component, Telegraph, Pather<Node> {
	...
	public CompAStar(EntityLiving entity, GridNodesAStar grid) {
		grid_nodes = grid;
		this.entity = entity;
		path = new GraphPath();
	}
	...
	public void createPath(float x, float y) {
		...
		if(Node.checkNode(grid_nodes, entity.getPosition().x, entity.getPosition().y)) {
			...
			recive = true;
			System.out.println("	compAStar: from node is out of grid!");
			return;
		}
		...
		if(Node.checkNode(grid_nodes, x, y)) {
			...
			//request = new PathFinderRequest<Node>(fromNode, fromNode, new HeuristicImp(), path);
			//request.client = this;
			
			//MessageManager.getInstance().dispatchMessage(this, entity.worldObj.pathFinder_queue, 0, request, false);
			PathfindingManager.getInstance().requestPathfinding(this, entity.worldObj.IASPF, fromNode, fromNode, path);
			System.out.println("	compAStar: to node is out of grid!");
			return;
		}
		
		//request = new PathFinderRequest<Node>(fromNode, toNode, new HeuristicImp(), path);
		//request.client = this;
		PathfindingManager.getInstance().requestPathfinding(this, entity.worldObj.IASPF, fromNode, toNode, path);
		
		//MessageManager.getInstance().dispatchMessage(0, request);
		//MessageManager.getInstance().dispatchMessage(this, entity.worldObj.pathFinder_queue, 0, request, false);
	}
	...
	public boolean handleMessage(Telegram msg) {
		...
		path = (GraphPath)((PathFinderRequest<Node>)msg.extraInfo).resultPath;
		recive = true;
