...
public class EditorCore implements ITraceBody {
	...
	public void saveLevel() {
		try {
			...
			for(int i = 0; i < Loader.getGameFile("assets/SJF/").list().length; i++) {
				System.out.println(Loader.getGameFile("assets/SJF/").list()[i]);
			}
			
			FileHandle file_level = Loader.getGameFile("assets/SJF/" + getWorld().getLevel().getLevelName() + level._SUFF);
