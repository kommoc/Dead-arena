...
public class RenderDAEntityMobZombie extends RenderEntityLiving {
	
	/** Body SCML sprite. */
	public Player sprite_player;
	/** Boots SCML sprite. */
	Player sprite_boots;
	
	DAEntityMobZombie entityLiving;
	
	int lastTimeLine = -1;
	
	protected float dif_angle = 0f;
	
	public RenderDAEntityMobZombie(DAEntityMobZombie par1, SpriteBatch batch, Drawer drawer) {
		super(par1, batch, drawer);
		entityLiving = par1;
		
		sprite_player = new Player(RenderEngine.getSCMLData("zombie"));
		sprite_player.setScale(Game.SCALE_WORLD_VALUE_FINAL);
		sprite_player.setAnimation("idle_agro");
		
		sprite_boots = new Player(RenderEngine.getSCMLData("zombie_feet"));
